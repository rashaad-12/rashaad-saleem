...
that sits *under* rather than *around*. A fractal-noise grain overlays the page at 3.5% so the flat
...
`getBoundingClientRect()`. A `<canvas>` is a *replaced element*: when absolutely positioned with
...
a *tonal* mismatch. It is composited onto `--color-plate` with `mix-blend-mode: screen` — screening
